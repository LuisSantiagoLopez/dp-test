/*
  # Optimize search function with performance improvements

  1. Changes
    - Added statement timeout (5s)
    - Increased work memory
    - Optimized query execution
    - Added error handling
    - Improved materialized CTEs
...
BEGIN
  ...
  WITH 
    ...
    exact_matches AS MATERIALIZED (
      ...
      FROM price_data p
      WHERE lower(unaccent(p.nombre_generico)) ILIKE ANY (
        array_append(
          array_agg('%' || word || '%') OVER (),
          '%' || normalized_terms || '%'
        )
