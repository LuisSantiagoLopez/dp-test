...
BEGIN
  ...
  WITH 
    search_tokens AS MATERIALIZED (
      SELECT unnest AS word, row_number() OVER () as token_id
      FROM unnest(string_to_array(normalized_terms, ' '))
    ...
    exact_matches AS MATERIALIZED (
      ...
      FROM price_data p, search_tokens st
      WHERE lower(unaccent(p.nombre_generico)) ILIKE '%' || st.word || '%'
        OR lower(unaccent(p.nombre_generico)) ILIKE '%' || normalized_terms || '%'
      LIMIT 5
    ),
